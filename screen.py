...
def screen_ligand(args):
    ...
    use_rdkit_coords = args.use_rdkit_coords if args.use_rdkit_coords != None else args.dataset_params[
        'use_rdkit_coords']
    #preprocess ligand
    if not os.path.exists(args.ligand_to_screen):
        raise ValueError(f'Path does not exist: {args.ligand_to_screen}')
    print(f'Trying to load {args.ligand_to_screen}')
    lig = read_molecule(args.ligand_to_screen, sanitize=True)
    if lig != None:  # read mol2 file if sdf file cannot be sanitized
        used_lig = args.ligand_to_screen
    if lig == None: raise ValueError(f'The ligand file could not be read')
    lig_graph = get_lig_graph_revised(lig, args.ligand_to_screen, max_neighbors=dp['lig_max_neighbors'],
