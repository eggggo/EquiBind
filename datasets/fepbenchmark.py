...
class FEPBenchmark(Dataset):
    ...
    def __init__(self, device='cuda:0',
        ...
        self.fepbenchmark_dir = 'data/FEPBenchmark'
    ...
    def process(self):
        log(f'Processing FEPBenchmark complexes and saving it to [{self.processed_dir}]')
        ...
        protein_num = 0
        complex_names = []
        for subdir in os.listdir(self.fepbenchmark_dir):
            for mol in os.listdir(os.path.join(self.fepbenchmark_dir, subdir)):
                if 'prepared' in mol:
                    rec_path = os.path.join(self.fepbenchmark_dir, subdir, mol)
            for mol in os.listdir(os.path.join(self.fepbenchmark_dir, subdir)):
                if 'ligand' in mol:
                    path = os.path.join(self.fepbenchmark_dir, subdir, mol)
                    lig = read_molecule(path, sanitize=True,
                    ...
                    ligs.append(lig)
                    complex_names.append(f'{subdir}/{mol}')
        ...
        #             continue
        #     else:
        #         lig = read_molecule(os.path.join(self.fepbenchmark_dir, name, f'{name}_ligand.sdf'), sanitize=True,
        #                             remove_hs=self.remove_h)
        #         if lig == None:  # read mol2 file if sdf file cannot be sanitized
        #             lig = read_molecule(os.path.join(self.fepbenchmark_dir, name, f'{name}_ligand.mol2'), sanitize=True,
        #                                 remove_hs=self.remove_h)
        #     if self.only_polar_hydrogens:
        #         for atom in lig.GetAtoms():
        #             if atom.GetAtomicNum() == 1 and [x.GetAtomicNum() for x in atom.GetNeighbors()] == [6]:
        ...
        # else:
        #     rec_paths = [os.path.join(self.fepbenchmark_dir, name, f'{name}_protein_processed.pdb') for name in
        #                  complex_names]
