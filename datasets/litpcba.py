...
class LitPCBA(Dataset):
    ...
    def process(self):
        log(f'Processing LitPCBA complexes from [{self.complex_names_path}] and saving it to [{self.processed_dir}]')
        ...
        for name in tqdm(complex_names, desc='loading ligands'):
            if self.bsp_ligands:
                ...
                path = name.split('/')
                lig = read_molecule(os.path.join(self.litpcba_dir, path[0], f'{path[1]}_ligand.mol2'), sanitize=True,
        ...
        if self.bsp_proteins:
            ...
            for name in complex_names:
                path = name.split('/')
                rec_paths.append(os.path.join(self.litpcba_dir, path[0], f'{(path[1])}_protein.pdb'))
